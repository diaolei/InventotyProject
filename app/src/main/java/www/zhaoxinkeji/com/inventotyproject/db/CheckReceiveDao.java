...
public class CheckReceiveDao {

    private SQLiteDatabase mDb;
    private List<Goods.ContentBean.GoodsBean> mList;
    private String formName;

    public CheckReceiveDao(Context c, String name) {
    ...
    public boolean insert(Goods.ContentBean.GoodsBean bean,String billNum){
        ContentValues values = new ContentValues();
            ...
            values.put(DbConst.GIFT_NUM, bean.getGifNum());
    ...
    public List<Goods.ContentBean.GoodsBean> queryContact(String billNum) {
        ...
        Cursor cursor = mDb.query(formName,
                ...
                DbConst.GOODS_NUM,DbConst.SCAN_NUM,DbConst.GIFT_NUM,
                        DbConst.GOODS_ID},
        ...
        while (cursor.moveToNext()) {
            ...
            String giftNum = cursor.getString(cursor.getColumnIndex(DbConst.GIFT_NUM));
    ...
    public boolean updateContact(String barcode,String newScan,String newGift) {
        ContentValues values=new ContentValues();
        values.put(DbConst.SCAN_NUM, newScan);
        values.put(DbConst.GIFT_NUM, newGift);
        //		update contactinfo set data='新数据' where data='原数据';
        //		whereClause 查询条件
        //		whereArgs 查询条件绑定的值
        int updateRows = mDb.update(formName, values, "barcode=?", new String[]{barcode});
        //		updateRows更新了3行  返回3
        return updateRows>0;
    }
}
