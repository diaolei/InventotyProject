...
import www.zhaoxinkeji.com.inventotyproject.adapter.CheckReceiveAdapter;
...
public class CheckReceiveActivity extends BaseActivity implements AdapterView.OnItemClickListener,MyListView.OnRefreshListener {

    private MyListView mCheckReceiveLv;
    private CheckReceiveAdapter mAdapter;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MessageConst.ACTION_INVENTORY :
                    mCheckReceiveLv.setAdapter(mAdapter);
                    if(mList.size()==mAdapter.mList.size()) {
                        mCheckReceiveLv.initcomplete();
                    }
                ...
                case MessageConst.ACTION_INIT_DIALOG :
                    MyDialog.getInstance().setContext(CheckReceiveActivity.this);
    ...
    @Override
    public void initUI() {
        mCheckReceiveLv = (MyListView) findViewById(R.id.check_receive_lv);
        mCheckReceiveLv.setOnItemClickListener(this);
        mCheckReceiveLv.setOnRefreshListener(this);
    }
    ...
    protected void sendAsyncTask() {
        ...
        RInventory rCheckReceive = JSON.parseObject(result, RInventory.class);

        if(rCheckReceive.getMsgInfo().equals("调用成功")) {
            for (int i = rCheckReceive.getContent().getBills().size()-1; i >=0; i--) {
                mList.add(rCheckReceive.getContent().getBills().get(i));
            }
            initAdapter();
            mHandler.obtainMessage(MessageConst.ACTION_INVENTORY).sendToTarget();
        }else{
            mHandler.obtainMessage(MessageConst.ACTION_INVENTORY_FAIL,rCheckReceive.getMsgInfo()).sendToTarget();
        }
    }
        ...
        mBuilder.setPositiveButton("确定",
                new DialogInterface.OnClickListener() {// 添加确定按钮
                    ...
                    public void onClick(DialogInterface dialog,
                                        int which) {// 确定按钮的响应事件
                        Intent intent = new Intent(CheckReceiveActivity.this,YsGoodsActivity.class);
    ...
    private void initAdapter() {
        mAdapter = new CheckReceiveAdapter(this, items);
    ...
    public void onPullRefresh() {
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                mAdapter.notifyDataSetChanged();
                if(mList.size()==mAdapter.mList.size()) {
                    mCheckReceiveLv.complete();
                }else{
                    mCheckReceiveLv.completeRefresh();
                }
            }
        },2000);
    }
