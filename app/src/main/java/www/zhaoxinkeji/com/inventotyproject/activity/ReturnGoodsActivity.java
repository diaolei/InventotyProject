...
public class ReturnGoodsActivity extends BaseActivity implements AdapterView.OnItemClickListener, MyListView.OnRefreshListener {

    private MyListView mReturnGoodsLV;
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case MessageConst.ACTION_INVENTORY :
                    mReturnGoodsLV.setAdapter(mAdapter);
                    if(mList.size()==mAdapter.mList.size()) {
                        mReturnGoodsLV.initcomplete();
                    }
                ...
                case MessageConst.ACTION_INIT_DIALOG :
                    MyDialog.getInstance().setContext(ReturnGoodsActivity.this);
    ...
    @Override
    public void initUI() {
        mReturnGoodsLV = (MyListView) findViewById(R.id.return_goods_lv);
        mReturnGoodsLV.setOnItemClickListener(this);
        mReturnGoodsLV.setOnRefreshListener(this);
    }
    ...
    protected void sendAsyncTask() {
        ...
        RInventory rReturnGoods = JSON.parseObject(result, RInventory.class);

        if(rReturnGoods.getMsgInfo().equals("调用成功")) {

            for (int i = rReturnGoods.getContent().getBills().size()-1; i >=0; i--) {
                mList.add(rReturnGoods.getContent().getBills().get(i));
            }

            initAdapter();
            mHandler.obtainMessage(MessageConst.ACTION_INVENTORY).sendToTarget();
        }else{
            mHandler.obtainMessage(MessageConst.ACTION_INVENTORY_FAIL,rReturnGoods.getMsgInfo()).sendToTarget();
        }
    }
        ...
        mBuilder.setPositiveButton("确定",
                new DialogInterface.OnClickListener() {// 添加确定按钮
                    ...
                    public void onClick(DialogInterface dialog,
                                        int which) {// 确定按钮的响应事件
                        Intent intent = new Intent(ReturnGoodsActivity.this,ThGoodsActivity.class);
    ...
    public void onPullRefresh() {
        new Handler().postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                mAdapter.notifyDataSetChanged();
                if(mList.size()==mAdapter.mList.size()) {
                    mReturnGoodsLV.complete();
                }else{
                    mReturnGoodsLV.completeRefresh();
                }
            }
        },2000);
    }
